...
<?php if ($postingan) : ?>
    ...
    <?php foreach ($postingan as $post) : ?>
        ...
        $infoPostingan = $this->db->get_where('user', ['id' => $id_user])->row_array();
                ...
                <div class="row">
                    <div class="col-sm-12 ">
                        <a class="text-decoration-none" href="<?= base_url('Diskusi/cekProfile/') . $id_user; ?>">
                            <img width="50px" height="50px" class="img-fluid rounded-circle img-thumbnail " src="<?= base_url('assets/user/img/user/') . $infoPostingan['image']; ?>" alt="">
                            <small class="text-muted  "><?= $infoPostingan['name']; ?></small>
                        </a>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <h5 class="card-title"><?= $infoPostingan['name']; ?> Menulis Ini &nbsp; <small style="font-size: 15px;" class="text-muted">Pada <?= date('h:i d F Y', $post['tanggal']); ?></small> </h5>
                ...
                <hr>
                <a href="<?= base_url('Diskusi/likePostinganPF/') . $post['id_diskusi'] . '/' . $user['id']; ?>" class="text-decoration-none ">
                ...
                <a href="<?= base_url('Diskusi/editDiskusiProfile/') . $post['id_diskusi']; ?>" class="text-decoration-none ">
                    Edit
                </a>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
